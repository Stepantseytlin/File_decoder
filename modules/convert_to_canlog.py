...
HEADER_RE = re.compile(
    ...
    r"\s*(?P=marker)(?P<year>\d{2})(?P<month>\d{2})\s*(?P<day>\d{2})"
)
    ...
    for line_number, (raw_line, next_raw_line) in enumerate(pairwise(lines), start=1):
        ...
        if match:
            ...
            current_header = (
                ...
                match.group("day") + "." + match.group("month")+"."+"20"+match.group("year"),
                line_number,
            )
            continue
    ...
    with input_path.open("r", encoding=encoding, errors="strict") as source, \
        ...
        for block in iter_blocks(source):
            ...
            for frame_index in range(0, len(block.words), 2):
                ...
                pl = f"{format_timestamp(timestamp)} " if (frame_count==0 and block_count==0) else f"\r\n{format_timestamp(timestamp)} "
                target.write(
                    f"{pl}"
