...
class LRU_CASH:
    ...
    def abort(self):
        if self.head.complete:
             return
        self.counter -=1
        print(f"aborting {self.head.name}\n")
        self.all_objects.pop(self.head.name)
        cleartail = False
        if self.head == self.tail: 
            cleartail = True

        self.head = self.head.to_tail
        if self.head: self.head.to_head=None
        if cleartail: self.tail = None
    def clear(self):
        self.counter = 0
        self.head : Cash_Object= None
        self.tail : Cash_Object = None 
